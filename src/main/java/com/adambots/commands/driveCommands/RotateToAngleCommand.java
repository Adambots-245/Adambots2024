...
public class RotateToAngleCommand extends Command {
  ...
  private Gyro gyro;
  private PIDController pidController = new PIDController(AutoConstants.kPThetaController, 0, AutoConstants.kDThetaController);
  private double drive_output;
  private double targetAngleRad;

  public RotateToAngleCommand(DrivetrainSubsystem driveTrainSubsystem, double targetAngleDeg, Gyro gyro) {
  ...
  public void execute() {
    ...
    drive_output = pidController.calculate(gyro.getContinuousYawRad(), targetAngleRad);
