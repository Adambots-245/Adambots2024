...
public class AlignWhileDrivingCommand extends Command {
  ...
  public AlignWhileDrivingCommand(DrivetrainSubsystem driveTrainSubsystem, IntakeSubsystem intakeSubsystem, CANdleSubsystem candleSubsystem, String limelight) {
  ...
  public void execute() {
    ...
    if (VisionHelpers.isDetected(limelight)){
      driveTrainSubsystem.drive(Buttons.forwardSupplier.getAsDouble()*DriveConstants.kMaxSpeedMetersPerSecond, Buttons.sidewaysSupplier.getAsDouble()*DriveConstants.kMaxSpeedMetersPerSecond , rotation_output, true);  
    } else {
